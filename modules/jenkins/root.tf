...
data "aws_iam_policy_document" "terraform_create_jenkins_document_b" {
  ...
  statement {
    ...
    resources = [
      ...
      "arn:aws:ssm:eu-west-2:${data.aws_caller_identity.current.account_id}:parameter/${var.environment}/integration_terraform_access_key",
      "arn:aws:ssm:eu-west-2:${data.aws_caller_identity.current.account_id}:parameter/${var.environment}/integration_terraform_secret_key",
